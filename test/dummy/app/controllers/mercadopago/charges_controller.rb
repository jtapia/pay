...
class Mercadopago::ChargesController < ApplicationController
  before_action :set_charge, only: [:show, :refund]

  def index
    @charges = Pay::Charge.joins(:customer).where(pay_customers: {processor: :mercadopago}).order(created_at: :desc)
  end
  ...
  def create
    ...
    redirect_to mercadopago_charge_path(charge)
  rescue Pay::Error => e
